...
def run_wheel_strategy(
    client: Client, account_hash: str, target_tickers: List[str]
) -> None:
    """
    The Wheel Strategy:
    ...
    try:
        positions_response = client.account_details(
            account_hash, fields="positions"
        ).json()
        raw_positions = positions_response.get("securitiesAccount", {}).get(
            "positions", []
        )
    ...
    for ticker in target_tickers:
        ...
        try:
            chain_json = client.option_chains(
                ticker, contractType="PUT", strikeCount=15
            ).json()
            ...
            logger.info(
                f"Delta: {best_put['delta']:.3f}, Premium (Mid): ${best_put['option_price']:.2f}"
            )

            # 3. Construct the STO Order using schwab-api order builder
            symbol = (
                best_put.name
            )  # OptionChainAnalyzer sets index to the exact Schwab symbol
            quantity = 1
...
def find_credit_spread(client: Client, account_hash: str, ticker: str) -> None:
    ...
    try:
        chain_json = client.option_chains(
            ticker, contractType="PUT", strikeCount=20
        ).json()
        df_chain = parse_option_chain_to_df(chain_json)
        chain = OptionChainAnalyzer(df_chain)

        # Get all puts expiring in around 30 days
        puts_30d = chain.filter_options(is_put=True, min_dte=25, max_dte=35)
        ...
        short_put_candidates = options[
            (options["delta"].abs() >= 0.25) & (options["delta"].abs() <= 0.35)
        ]
        ...
        logger.info(
            f"  Short Leg: Sell {short_put.name} (${short_put['strike_price']} strike) at ${short_put['bid']:.2f}"
        )
        logger.info(
            f"  Long Leg:  Buy  {long_put.name} (${long_put['strike_price']} strike) at ${long_put['ask']:.2f}"
        )
        logger.info(
            f"  Max Risk:  ${(short_put['strike_price'] - long_put['strike_price'] - net_credit) * 100:.2f}"
        )
